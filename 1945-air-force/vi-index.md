...
<img src="assets/ux/dashboard.jpeg" alt="Dashboard chính: Quick Play, squadron/loadout, event và offer cùng nằm trên hub" width="500">
...
Phần dưới phân tích từng lớp của vòng lặp này.
...
Game không bắt học nhiều trước khi thấy vui. Chỉ cần hiểu một hành động: di chuyển máy bay để sống sót. Auto-fire xử lý phần tấn công, nên độ khó tập trung vào đọc đạn, tìm gap an toàn và né đúng thời điểm.
...
Mỗi màn chơi = một bước nhỏ: thêm resource, thêm power, tiến gần hơn tới content khó hơn.
...
<img src="assets/combat/gameplay_battle.jpeg" alt="In-battle HUD: HP, timer, device skill, aircraft/wingman và bullet pattern" width="500">
...
<img src="assets/combat/hitbox_example.png" alt="Hitbox nhỏ hơn sprite: cánh có thể lướt qua đạn, phần thân mới là vùng nguy hiểm chính" width="500">
...
<img src="assets/loadout/aircraft-infomation.png" alt="Aircraft detail: star, stats, gear slots, engine và certificate trên một unit" width="500">
...
<img src="assets/loadout/wingman-information.png" alt="Wingman screen: support unit có gear/engine riêng, mở thêm một bề mặt đầu tư ngoài aircraft" width="500">

<img src="assets/loadout/device-information.png" alt="Device screen: utility/burst support cũng có progression riêng" width="500">
...
Combat core không cần thay đổi nhiều nhưng game vẫn sâu hơn theo thời gian nhờ loadout system. Rủi ro: complexity tăng nhanh — mở quá nhiều hệ thống sớm sẽ làm dashboard thành ma trận.
...
<img src="assets/progression/upgrade-main-gun.png" alt="Gear upgrade: nâng cấp Main Gun bằng resource, một trong nhiều đường tăng power" width="500">

Nếu chỉ có một đường upgrade, người chơi nhanh chạm trần. Nhiều đường upgrade giúp khi một đường chậm lại, vẫn còn đường khác để theo.
...
<img src="assets/progression/upgrade-engine.png" alt="Engine upgrade: thêm một trục power dùng material riêng và cạnh tranh với gear" width="500">

<img src="assets/progression/basic-certificate.png" alt="Certificate: chuỗi mục tiêu theo aircraft, biến breadth of play thành power lâu dài" width="500">
...
Battle tạo resource → resource tiêu vào progression → thiếu resource → game mở ra daily/event/shop/ad/gacha → quay lại hoặc chi tiền.

<img src="assets/economy/campaign-rewards.png" alt="Campaign reward: battle result trả Gold, Gem, Honor Point và Modules để đổ vào progression" width="500">

Mỗi resource có vai trò riêng nên game điều tiết progression tốt. Rủi ro: cognitive load — người chơi mới khó phân biệt Chips, Beer, Wrenches, Batteries, Modules và không biết farm cái nào trước.
...
<img src="assets/retention/daily-gift.png" alt="Daily Gift: login reward tạo lý do quay lại theo chu kỳ" width="500">
...
<img src="assets/retention/event-independence-day.png" alt="Seasonal event: event currency + timer + exchange shop tạo FOMO và mục tiêu ngắn hạn" width="500">
...
Mua bất kỳ IAP nào (kể cả \$0.99) giảm hoặc bỏ interstitial → lần mua đầu vừa là QoL upgrade vừa là power boost.

Revive: chết giữa trận → 2 option: Gems hoặc xem ad (max 5 ad/ngày). Conversion cao vì người chơi đã đổ thời gian vào run đó.

<img src="assets/monetization/free.png" alt="Free tab: rewarded ads được đóng gói thành crate/reward loop" width="500">
...
<img src="assets/monetization/vip.png" alt="VIP privilege: lifetime spending ladder biến chi tiêu thành power và resource advantage dài hạn" width="500">
...
Các system ở mục 3 tương tác với nhau: system nào trigger system nào, người chơi bị dẫn đi đâu tại mỗi điểm.
...
1945 Air Force có 900+ campaign level × 3 difficulty = 2700+ variation. Không thể handcraft từng level — game dùng hệ thống template + scaling.

### 5.1 Enemy Pool

Game xây dựng một pool enemy phân theo role:

| Role    | Chức năng trong level                                             |
| ------- | ----------------------------------------------------------------- |
| Grunt   | HP thấp, lấp đầy màn hình, cho người chơi kill rhythm + loot      |
| Elite   | HP cao hoặc đạn dày hơn, buộc ưu tiên target                      |
| Special | Thay đổi luật chơi: homing, shield, kamikaze, bullet pattern nặng |
| Boss    | Pattern-learning, climax của level, DPS check                     |

Mỗi level là một tổ hợp từ pool này — thay đổi số lượng, loại và thứ tự spawn để tạo level mới mà không cần thiết kế từ đầu.

### 5.2 Wave Composition

Mỗi level có cấu trúc wave cố định:

```text
Opening wave → áp lực thấp, warm-up, grunt chủ yếu
Mid waves    → mật độ tăng, mix grunt + elite + special, power-up xuất hiện
Pre-boss     → áp lực cao nhất, ít khoảng nghỉ, special enemy nhiều
Boss         → target lớn, pattern lặp lại, mastery check
```

Pacing xen kẽ tension và relief — nếu wave nào cũng dày thì người chơi burn out, wave nào cũng nhẹ thì chán. 1945 Air Force dùng burst áp lực → khoảng thở ngắn → burst tiếp.

### 5.3 DPS Threshold

Mỗi level có tổng HP enemy + time limit → tạo ra **DPS threshold** (lượng damage/giây tối thiểu để clear).

Cơ chế:

- Level designer set: enemy HP, số lượng, time limit
- Progression designer set: power curve (ATK từ upgrade, gear, engine, tier)
- Khoảng cách giữa DPS threshold và player power = **progression wall**

Player power < DPS threshold → không clear kịp → fail → cần upgrade. Hệ thống power (ATK, gear damage%, engine bonus) tồn tại để phục vụ phép đo này.

Time limit ở difficulty cao siết chặt hơn: nếu DPS không đủ, enemy thoát hoặc objective fail → mất 3-star dù dodge giỏi.

### 5.4 Difficulty ×3

Cùng 1 level có 3 mức difficulty (Easy / Medium / Hard), scale theo 5 chiều:

| Chiều                | Easy                       | Hard                             |
| -------------------- | -------------------------- | -------------------------------- |
| Enemy HP/damage      | Chịu được 4-5 hit          | 1-2 hit chết                     |
| Bullet density/speed | Gap rộng, né thoải mái     | Gap pixel-level, bullet dày      |
| One-hit kill threats | Ít                         | Nhiều (homing, laser, lightning) |
| Boss complexity      | Ít phase, pattern đơn giản | Thêm attack type, phase ngắn hơn |
| Time pressure        | Thoải mái                  | DPS check chặt                   |

Difficulty scale cả skill (né pattern khó hơn) lẫn power (cần DPS đủ). Ở Hard, skill giỏi không bù được gear yếu → buộc upgrade.

### 5.5 Boss Design

Boss là climax của level. Người chơi phải đọc pattern, tìm gap và né dưới áp lực liên tục.

**Boss 1 — Battleship (early campaign)**

<img src="assets/level-design/boss-1.png" alt="Boss 1: Battleship với nhiều turret, arc bullet + homing missile" width="500">

Tàu chiến lớn chiếm nửa trên màn hình. Turret cố định → người chơi học được nguồn đạn. 2 loại attack: arc/spread bullet từ turret + homing missile. Turret phá được từng cái → giảm mật độ đạn khi fight kéo dài.

Pattern đơn giản, đọc được. Người chơi học đọc nguồn đạn và ưu tiên positioning từ boss này.

**Boss 2 — Large Bomber (Level 20)**

<img src="assets/level-design/boss-2-phase-1.png" alt="Boss 2 Phase 1: straight-line bullet từ nhiều turret" width="500">

Phase 1: nhiều turret bắn straight-line liên tục. Bullet speed cố định → học rhythm, tìm gap.

<img src="assets/level-design/boss-2-phase-2.png" alt="Boss 2 Phase 2: laser beam + accelerating bullet" width="500">

Phase 2: thêm laser sweep (có telegraph đỏ trước khi bắn) + accelerating bullet từ turret bên. Escalation từ Boss 1: thêm phase, thêm loại threat, nhưng vẫn giữ telegraph để đọc được.
...
Phase 1: turret giữa bắn 3 viên dừng tại chỗ (zone denial) + 2 turret bên bắn straight-line. Phải né stream song song + tránh vùng cấm giữa.

<img src="assets/level-design/boss-3-phase-2.png" alt="Boss 3 Phase 2: homing missile + laser targeting" width="500">

Phase 2: thêm 2 homing missile có laser tracking. Né bullet Phase 1 + 2 missile cùng lúc.

<img src="assets/level-design/boss-3-phase-3.png" alt="Boss 3 Phase 3: bomb cross-pattern + thêm turret" width="500">

Phase 3: bomb turret bắn vào vị trí người chơi → nổ thành cross-pattern xoay chậm + 2 turret phụ bắn single-fire.

**Escalation pattern qua 3 boss:**

|              | Boss 1       | Boss 2                      | Boss 3                            |
| ------------ | ------------ | --------------------------- | --------------------------------- |
| Phase        | 1            | 2                           | 3                                 |
| Bullet type  | Arc + homing | Straight + laser + accel    | Zone denial + homing + bomb cross |
| Mechanic mới | —            | Phase transition, telegraph | Zone denial, layered threat       |

Mỗi boss thêm 1 mechanic layer mới, giữ mechanic cũ readable. Complexity tăng nhưng người chơi vẫn hiểu vì sao mình chết.

### 5.6 Scale 900+ Level

Cơ chế core để tạo 900+ level từ hệ thống template:

1. **Enemy pool** → mix role khác nhau cho mỗi level
2. **Stat scaling** → cùng enemy type, HP/damage tăng theo level number
3. **Boss rotation** → boss pool được reuse, scale stats, thêm phase ở level cao
4. **×3 difficulty** → 1 level × 3 mức = 3 variation
5. **Mechanic drip** → enemy type mới xuất hiện dần qua campaign (homing → shield → laser → kamikaze...)
6. **Power gate** → DPS threshold tăng → buộc upgrade → gắn với progression system

Game thiết kế **bộ công cụ tạo level** (enemy pool, wave rule, scaling formula, boss pattern library) — không ngồi vẽ từng level. Chi phí sản xuất thấp, output cao.

Lưu ý: phân tích trên dựa trên observation từ 30+ level đầu và evidence thu thập được. Cơ chế chính xác (formula scaling, procedural hay manual config) cần access source hoặc chơi sâu hơn để verify.
...
Ngay sau result, popup Rank Up Offer xuất hiện — đề nghị upgrade star bằng Gems. Monetization trigger gắn vào thời điểm người chơi vừa xong trận, muốn tiến nhanh.
...
Aircraft, Wingman, Device, Pilot, Gear, Engine, Certificate tạo ra nhiều nơi tiêu resource. Reward sau battle luôn có chỗ để đổ.
...
1945 Air Force là ví dụ rõ của mô hình F2P mobile lâu dài: combat đơn giản giữ barrier thấp, meta progression sâu giữ người chơi đầu tư, economy routing dẫn hành vi, và monetization gắn vào nhu cầu thật thay vì ép mua.

Game không làm tốt mọi thứ — popup offer quá nhiều, dashboard late-game clutter, VIP compound advantage ảnh hưởng competitive fairness. Nhưng cấu trúc core loop → progression → economy → retention → monetization được tổ chức chặt và có thể áp dụng cho nhiều thể loại F2P khác, không riêng shmup.

Bản deconstruction này phân tích dựa trên observation từ ~30 level đầu campaign, evidence thu thập từ in-game screenshots và community data. Một số cơ chế (scaling formula, late-game content, PvP/Clan/Battleship flow) cần chơi sâu hơn để verify đầy đủ.
